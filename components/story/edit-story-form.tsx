...
import { useCreateStoryForm, InitialStoryData } from "@/hooks/story/use-create-story-form";
...
interface EditStoryFormProps {
  slug: string;
  initialData: InitialStoryData & { tags: string[], imageUrls: string[] };
}

export function EditStoryForm({ slug, initialData }: EditStoryFormProps) {
  const form = useCreateStoryForm(initialData);
  const { trails, loading: loadingTrails } = useTrails();
  const tagManager = useTagManager(initialData.tags);
  const imageManager = useImageManager(initialData.imageUrls);
  const { isSubmitting, isDraft, error, setError, handleUpdate } =
    useSubmitStory();
    ...
    <>
      ...
      >
        ...
        <PrivacyActionsSection
          ...
          title={form.title}
          content={form.content}
          onSaveDraft={() => handleUpdate(slug, true, submitParams)}
          onPublish={() => handleUpdate(slug, false, submitParams)}
        />
      </motion.div>
    </>
  );
