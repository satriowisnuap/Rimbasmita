...
export function TrailSelect({ trails, value, onValueChange, disabled }: Props) {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          <div className="flex items-center gap-2 overflow-hidden">
            <Mountain className={cn("h-4 w-4 shrink-0", value ? "text-primary" : "text-muted-foreground")} />
            <span className="truncate">
              {selectedTrail 
                ? `${selectedTrail.name}${selectedTrail.location ? ` - ${selectedTrail.location}` : ""}`
      ...
      <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-0 rounded-xl border-border bg-popover/95 backdrop-blur-sm" align="start">
        <Command className="bg-transparent">
          <div className="flex items-center border-b border-border px-3">
            <Search className="mr-2 h-4 w-4 shrink-0 opacity-50" />
            <CommandInput placeholder="Search trail name or location..." className="h-11 bg-transparent outline-none border-none focus:ring-0" />
          </div>
          <CommandList className="max-h-[300px] overflow-y-auto">
            <CommandEmpty>No trail found.</CommandEmpty>
            <CommandGroup>
              {trails.map((trail) => (
                <CommandItem
                  key={trail.id}
                ...
                >
                  <div className={cn(
                    ...
                    value === trail.id ? "bg-primary text-primary-foreground" : "opacity-50"
                  )}>
